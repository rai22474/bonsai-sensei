...
COMPOSE_FILE="knowledge_base/tests/acceptance/docker-compose.acceptance.yml"
ACCEPTANCE_PORT="8060"
LOG_FILE="${ROOT_DIR}/knowledge_base/tests/acceptance/docker-compose.logs.txt"
TEST_TARGET="${1:-all}"
SCENARIO_FILTER="${2:-}"
...
ready=false
for i in {1..10}; do
  echo "Running: readiness check attempt ${i} (GET /api/wiki/files)"
  if curl -sf "http://localhost:${ACCEPTANCE_PORT}/api/wiki/files" >/dev/null; then
...
if [ "$TEST_TARGET" = "all" ]; then
  PYTEST_TARGET="knowledge_base/tests/acceptance"
else
  PYTEST_TARGET="$TEST_TARGET"
fi

PYTEST_LOG="${ROOT_DIR}/knowledge_base/tests/acceptance/pytest.log"
...
if ! run_step "pytest ${PYTEST_TARGET}" env ACCEPTANCE_API_BASE="http://localhost:${ACCEPTANCE_PORT}" uv run --project "${ROOT_DIR}/knowledge_base" pytest "${PYTEST_ARGS[@]}" 2>&1 | tee "$PYTEST_LOG"; then
  run_step "docker compose logs" save_logs
  echo "Docker logs saved to ${LOG_FILE}."
