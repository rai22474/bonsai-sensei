...
def create_ask_poll(
    send_poll_func: Callable,
    pending_responses: dict,
    poll_id_to_user_id: dict,
    timeout_seconds: int = DEFAULT_TIMEOUT_SECONDS,
    send_message_func: Callable | None = None,
) -> Callable:
    async def ask_poll(
        question: str,
        options: list[str],
        ...
        """Present a clarification question as a native Telegram poll with LLM-generated options.

        The last option in the poll is always a free-text fallback. If the user
        selects it, they are prompted to type their answer instead.

        Args:
            question: The clarification question (max 300 characters).
            options: Short option labels generated by the LLM (max 6, each max 90 chars).
            tool_context: ADK tool context providing the user identifier.

        Returns:
            The selected option text, or the user's free-text response if they chose the fallback.
        """
        user_id = resolve_confirmation_user_id(tool_context)
        event = asyncio.Event()
        pending_responses[user_id] = {
        ...
        poll_id = await send_poll_func(user_id, question, options)
        if poll_id:
            poll_id_to_user_id[poll_id] = user_id
            pending_responses[user_id]["poll_id"] = poll_id
        try:
            await asyncio.wait_for(event.wait(), timeout=timeout_seconds)
        except TimeoutError:
            poll_id_to_user_id.pop(poll_id, None)
            pending_responses.pop(user_id, None)
            return ""
        poll_id_to_user_id.pop(poll_id, None)
        return pending_responses.pop(user_id)["response"]
