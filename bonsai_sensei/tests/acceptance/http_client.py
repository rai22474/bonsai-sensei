...
KB_BASE_URL = os.getenv("ACCEPTANCE_KB_BASE", "http://localhost:8061")
...
async def _kb_request_json_async(method: str, path: str, payload: dict | None = None):
...
def get_kb(path: str):
    return asyncio.run(_kb_request_json_async("GET", path))


def post_kb(path: str, payload: dict | None = None):
    return asyncio.run(_kb_request_json_async("POST", path, payload))


def put_kb(path: str, payload: dict | None = None):
    return asyncio.run(_kb_request_json_async("PUT", path, payload))


def delete_kb(path: str):
    return asyncio.run(_kb_request_json_async("DELETE", path))
...
async def post_bonsai_photo_async(bonsai_id: int, photo_bytes: bytes, filename: str = "photo.png", taken_on: str | None = None) -> dict:
    params = {}
    if taken_on is not None:
        params["taken_on"] = taken_on
...
def post_bonsai_photo(bonsai_id: int, photo_bytes: bytes, filename: str = "photo.png", taken_on: str | None = None) -> dict:
    return asyncio.run(post_bonsai_photo_async(bonsai_id, photo_bytes, filename, taken_on))
