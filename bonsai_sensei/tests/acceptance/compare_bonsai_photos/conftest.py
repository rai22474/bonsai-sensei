...
from http_client import delete, delete_kb, get, post, post_bonsai_photo
...
def context():
    return {
        "user_id": f"bdd-compare-{uuid.uuid4().hex}",
        "bonsai_created": [],
        "species_created": [],
        "bonsai_ids": {},
        "species_ids": {},
    ...
    for name in context["bonsai_created"]:
        ...
        if bonsai:
            ...
            if bonsai_photos:
                ...
                report_path = f"users/{context['user_id']}/bonsai/{slug}/reports/{newest_taken_on}-comparison.md"
                try:
                    delete_kb(f"/api/wiki?path={report_path}")
